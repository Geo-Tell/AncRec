...
def nms_3d_faster_samecls(boxes, overlap_threshold, old_type=False):
    ...
    score = boxes[:,6]
    cls = boxes[:,7]
    area = (x2-x1)*(y2-y1)*(z2-z1) # N x 1

    I = np.argsort(score)
    pick = []
    while (I.size!=0):
        last = I.size  #从score最大的开始选
        i = I[-1]
        pick.append(i)
        ...
        if old_type:
            ...
            inter = l*w*h
            o = inter / (area[i] + area[I[:last-1]] - inter)
        o = o * (cls1==cls2) #如果类别不同，就认为他们没有重叠了

        I = np.delete(I, np.concatenate(([last-1], np.where(o>overlap_threshold)[0])))

    return pick

def nms_3d_cls_conditioned(boxes, overlap_threshold, old_type=False):
    x1 = boxes[:,0]
    y1 = boxes[:,1]
    z1 = boxes[:,2]
    x2 = boxes[:,3]
    ...
    while (I.size!=0):
        ...
        for i in range(len(cls2)):
            cls2_ = cls2[i]
            if {cls1, cls2_} == {0, 1} or {cls1, cls2_} == {0, 4} or {cls1, cls2_} == {1,4}:
                # 如果是桌子和椅子、或者桌子和垃圾桶，或者椅子和垃圾桶，则可以完全重叠
                o[i] = 0

...
from net_utils.box_util import get_3d_box2
...
def box_collision_test(boxes, qboxes, clockwise=True):
    ...
    boxes_standup = box_util.corner_to_standup_nd_jit(boxes)
    qboxes_standup = box_util.corner_to_standup_nd_jit(qboxes)
    for i in range(N):
        for j in range(K):
...
def noise_per_box(boxes, valid_mask, loc_noises, rot_noises):
    ...
    box_corners = box_util.box2d_to_corner_jit(boxes)
...
def noise_per_object_v3_(gt_boxes,
    ...
    origin = (0.5, 0.5, 0)
    # convert locations, sizes and angles to corners
    gt_box_corners = get_3d_box2(
        gt_boxes[:, :3],
        gt_boxes[:, 3:6],
        gt_boxes[:, 6],
        origin=origin,
    ...
    # collision test: only needs xy coordinates
    if not enable_grot:
    ...
    surfaces = box_util.corner_to_surfaces_3d_jit(gt_box_corners)
    if points is not None:
        # TODO: replace this points_in_convex function by my tools?
        point_masks = box_util.points_in_convex_polygon_3d_jit(
            points[:, :3], surfaces)
...
if __name__ == '__main__':
    scan_name = 'scene0339_00'
    box_pth = 'bbox.pkl'
    scan_pth = 'full_scan.npz'
    scan_data = np.load(os.path.join('/home/dmy/indoor3D/data/datasets/scannet/processed_data', scan_name, scan_pth))
    point_cloud = scan_data['mesh_vertices'][:,0:3]
    boxes3D = []
    with open(os.path.join('/home/dmy/indoor3D/data/datasets/scannet/processed_data', scan_name, box_pth), 'rb') as file:
    ...
    # write_ply(point_cloud, 'pc.ply')
    # write_oriented_bbox(boxes3D, 'boxes.ply')


    noise_per_object_v3_(boxes3D,points=point_cloud, global_random_rot_range = 0)
    write_ply(point_cloud, 'mew_pc.ply')
    write_oriented_bbox(boxes3D, 'new_boxes.ply')

...
from configs.path_config import scannet_processed_path, posed_images_path, split_path
import os
import numpy as np
import pickle
...
if __name__ == '__main__':
    ...
    for iter, scan_name in enumerate(scan_names):
        ...
        data['ptids_images'] = [ptids_images]
        est_data = tester.net.module.generate(data)
        tester.save_results(iter, data, est_data, dump_dir,save_gt=False)
        save_scene_mesh(dump_dir, scan_name, dump_scene_mesh_dir, align_to_gt=False)
        #requirement: vis_bbox.npy; proposal_ids.npy; obj_probs.npy
        #pred_meshes_format: 'target_(\d+)_proposal_(\d+)_class_(\d+)_mesh.ply'

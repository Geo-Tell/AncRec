...
class FinetuneByRGB(nn.Module):
    def __init__(self, cfg, optim_spec=None):
        super(FinetuneByRGB, self).__init__()
    ...
    def forward(self, data, anchors, augment=True, return_images = True):
        ...
        # output_images = []
        # cropped_image_default = torch.zeros(1, self.image_size, self.image_size, 3)
        ...
        for bid in range(batch_size):
            ...
            for nid in range(nproposal):
                ...
                '''找到proposal对应的instance id (出现点次数最多的instance)'''
                ...
                best_img_id = -1
                MIN_NUM_PTS_THRESH = 200 if augment else 400
                ...
                output_pts2d = np.concatenate([ids_image_x, ids_image_y]).astype(np.int).reshape(2,-1).transpose()  # N,2
            ...
            for nid in range(nproposal):
                ...
                for img_id, ptids_image in enumerate(ptids_images[bid]):  # [max_count_id]
                    ...
                    if num_pts_in_image > best_num_pts_in_image:
                        ...
                        best_img_id = img_id
                        best_ptids_image = ptids_image
                        # best_ptids_image_flattened = ptids_image_flattened
                        # best_ids_pc_mask = ids_pc_mask                if best_num_pts_in_image < self.min_num_pts:

...
class CResnetBlockConv1d(nn.Module):
    ...
    def __init__(self, c_dim, size_in, size_h=None, size_out=None, bn_tracking_runnin_status = True,
        ...
        self.bn_0 = CBatchNorm1dtest(c_dim, size_in, bn_tracking_runnin_status)
    ...
    def forward(self, x, c, x_test=None):
        net = self.fc_0(self.actvn(self.bn_0(x, c, x_test)))
...
class CBatchNorm1dtest(nn.Module):
    ''' Conditional batch normalization layer class.
    ...
    def __init__(self, c_dim, f_dim, use_running_mean_var = True):
        super().__init__()
        self.c_dim = c_dim
        self.f_dim = f_dim
        # Submodules
        self.bn = nn.BatchNorm1d(f_dim, affine=False, track_running_stats = use_running_mean_var)
        self.conv_gamma = nn.Conv1d(c_dim, f_dim, 1)
        self.conv_beta = nn.Conv1d(c_dim, f_dim, 1)
        self.reset_parameters()
    ...
    def forward(self, x, c, x_test = None):
        ...
        if x_test is not None:
            c_dim = x.shape[1]
            # if self.bn_test.training:
            mean_x_test = x_test.transpose(1, 2).reshape(-1, c_dim).mean(0)
            var_x_test = x_test.transpose(1, 2).reshape(-1, c_dim).var(0)
            net = (x - mean_x_test.unsqueeze(0).unsqueeze(-1).repeat(x.shape[0], 1, x.shape[-1])) / torch.sqrt(
                var_x_test.unsqueeze(0).unsqueeze(-1).repeat(x.shape[0], 1, x.shape[-1]) + 1e-5)
...
class ResnetPointnet(nn.Module):
    ...
    def __init__(self, config, optim_spec): #c_dim=128, dim=3, hidden_dim=128):
        super().__init__()
        c_dim = config['c_dim']
        hidden_dim = config['hidden_dim']
        use_rgb_features = config.get('use_rgb_features',False)
        dim = 256 * config['use_proposal_features'] + 256 * use_rgb_features +3
        self.c_dim = c_dim
        ...
        self.fc_c = nn.Linear(hidden_dim, c_dim) #512,256
        self.actvn = nn.ReLU()
        self.pool = maxpool

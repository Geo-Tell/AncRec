...
def load_scheduler(config, optimizer):
    ...
    method = config['scheduler'].get('method', None)
    if method and method == 'ReduceLROnPlateau':
        ...
        scheduler = torch.optim.lr_scheduler.ExponentialLR(optimizer, 0.9) #gamma=1.05) #
    # scheduler = PolynomialLRDecay(optimizer, max_decay_steps=20000, end_learning_rate=0.0001, power=2.0) #total: 42660
...
def load_optimizer(config, net):
    ...
    # elif config['optimizer']['method'] == 'Ranger':
    #     optimizer = Ranger(optim_params,
    #                        lr=float(default_optim_spec['lr']),
    #                        betas=tuple(default_optim_spec['betas']),
    #                        eps=float(default_optim_spec['eps']),
    #                        weight_decay=float(default_optim_spec['weight_decay']))

    elif config['optimizer']['method'] == 'AdamW':

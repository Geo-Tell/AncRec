...
class BaseNetwork(nn.Module):
    ...
    def load_weight_from_others(self, pretrained_model, new_prefix=None, old_prefix =None
        ...
        for k, v in pretrained_model.items():
            ...
            if old_prefix is not None:
                k_new = k.replace(old_prefix + '.', '')
            else:
                k_new = k
            submodule_prefix = k.split('.')[0]
            if submodule_prefix in load_module:
               if new_prefix is not None:
                  k_new = new_prefix + '.' + k_new
               if k_new in model_dict:
                    pretrained_dict[k_new] = v
               else:
                    self.cfg.log_string('unknown keys:' + str(k_new))
            else:
                self.cfg.log_string('submodule_prefix {} is not in {}'.format(submodule_prefix,load_module))

        self.cfg.log_string(
            str(set([key_new.split('.')[0] for key_new in model_dict if key_new not in pretrained_dict])) + ' subnet missed.')

...
def eval(gt_dir, pred_dir, voxel_size, threshs=[0.5, 0.25], conf_thresh=0.5, sem_thresh = 0.):
    log_file = open(pred_dir +'/' + 'log_iou_obj%s_sem%s.txt'%(conf_thresh,sem_thresh), 'w') #a
    ...
    for sid, scene_name in enumerate(scene_names):
        ...
        for f in pred_files_scene:
            ...
            score = obj_score * sem_score
            if obj_score > conf_thresh and sem_score > sem_thresh:  #
...
if __name__ == '__main__':
    ...
    eval(args.gt_dir, args.pred_dir, args.voxel_size,conf_thresh=0.05, sem_thresh = 0.5)

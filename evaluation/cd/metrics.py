...
# raw_label_map_file = '/home/dmy/indoor3D/data/datasets/scannet/rfs_label_map.csv'
...
def eval_det_cls_w_mesh(pred, gt, thresh, use_07_metric):
    ...
    return (rec_mesh[-1], prec_mesh[-1], ap_mesh, fp_mesh, tp_mesh), (PQ_mesh, SQ_mesh, RQ_mesh)
...
def eval_det_cls_wrapper_w_mesh(arguments):
    ...
    (rec_mesh, prec_mesh, ap_mesh, fp_mesh, tp_mesh), (PQ_mesh, SQ_mesh, RQ_mesh) = eval_det_cls_w_mesh(pred, gt, thresh, use_07_metric)
    return (rec_mesh, prec_mesh, ap_mesh, fp_mesh, tp_mesh), (PQ_mesh, SQ_mesh, RQ_mesh)
...
def eval_det_multiprocessing_w_mesh(pred_all, gt_all, thresh=0.25, use_07_metric=True):
    ...
    for i, label in enumerate(gt.keys()):
        if label in pred:
            (rec_mesh[label], prec_mesh[label], ap_mesh[label],fp_mesh[label], tp_mesh[label]), (PQ_mesh[label], SQ_mesh[label], RQ_mesh[label]) = ret_values[i]
        else:
            (rec_mesh[label], prec_mesh[label], ap_mesh[label],fp_mesh[label], tp_mesh[label]), (PQ_mesh[label], SQ_mesh[label], RQ_mesh[label]) = (0, 0, 0,0,0), (0, 0, 0)
...
class APCalculator(object):
    ...
    def compute_metrics(self):
        ...
        (rec_mesh, prec_mesh, ap_mesh, fp_mesh, tp_mesh), (PQ_mesh, SQ_mesh, RQ_mesh) = eval_det_multiprocessing_w_mesh(self.pred_map_cls, self.gt_map_cls, thresh=self.ap_iou_thresh)
        ...
        fp_total_mesh = 0
        tp_total_mesh = 0
        for key in sorted(fp_mesh.keys()):
            clsname = self.class2type_map[key] if self.class2type_map else str(key)
            ret_dict['mesh false positive number %s ' % (clsname)] = fp_mesh[key]
            ret_dict['mesh true positive number %s ' % (clsname)] = tp_mesh[key]
            ret_dict['mesh false positive ratio %s' % (clsname)] = fp_mesh[key]/ (tp_mesh[key]+ fp_mesh[key])

            fp_total_mesh += fp_mesh[key]
            tp_total_mesh += tp_mesh[key]


        ret_dict['mesh total false positive number'] = fp_total_mesh
        ret_dict['mesh total true positive number'] = tp_total_mesh
        ret_dict['mesh total false positive ratio'] = fp_total_mesh / (fp_total_mesh + tp_total_mesh)

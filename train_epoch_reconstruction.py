...
import torch
import os
# from models.anchorrec.dataloader import  get_shape_frequencies, SingleObject, collate_fn,my_worker_init_fn
...
# from models.anchorrec.dataloader import SingleObject, collate_fn,my_worker_init_fn  SingleObject_gt, SingleObject_instance_gt,
        ...
        if (iter % cfg.config['log']['vis_step_train']) == 0 and cfg.config['log']['save_results'] == True \
            ...
            for bid, mesh in enumerate(est_data['meshes']):
                ...
                if 'gt_meshes' in est_data:
                    est_data['gt_meshes'][bid].export(os.path.join(vis_path,  'train_epoch_%s_iter_%s_gt_z_%s.ply' % (epoch, iter, bid)))
                # gt mesh
                ...
                write_ply(data['object_points'][bid].detach().cpu().numpy(), os.path.join(vis_path,
                                                'train_epoch_%s_iter_%s_objpts_%s.ply' % (epoch, iter, bid)))

                # write_ply(data['original_object_points'][bid].detach().cpu().numpy(), os.path.join(vis_path,
                #                                  'train_epoch_%s_iter_%s_original_objpts_%s.ply' % (epoch, iter, bid)))

                '''input point cloud (save only once for each scene)'''
                # with open(os.path.join(vis_path, 'train_epoch_%s_iter_%s_bid_%s_%s.txt' % (epoch, iter, bid, data['scan_name'][bid])),'w'):
                #     pass
                # scene_pc = \
                # np.load(os.path.join(PathConfig.processed_data_path, data['scan_name'][bid], 'full_scan.npz'))[
                #     'mesh_vertices'][:, :3]
                ...
                '''input images'''
                cv2.imwrite(os.path.join(vis_path, 'train_epoch_%s_iter_%s_img_%s.jpg' % (epoch, iter, bid)),
                            denormalize_image(data['img'][bid]))
...
def train(cfg, trainer, scheduler,checkpoint, validate = False):
    ...
    train_dataset = SingleObject_recon_new(cfg, 'train')
    ...
    if validate:
        val_dataset = SingleObject_recon_new(cfg, 'val')
    ...
    for iter, data in enumerate(dataloader):
        ...
        if (iter % cfg.config['log']['vis_step_val']) == 0 and cfg.config['log']['save_results'] == True \
            ...
            for bid, mesh in enumerate(est_data['meshes']):
                ...
                if 'img' in data:
                    cv2.imwrite(os.path.join(vis_path, 'val_epoch_%s_iter_%s_img_%s.jpg' % (epoch, iter, bid)),
                                denormalize_image(data['img'][bid]))

...
def run(cfg, mode):
    ...
    eval_map = True if mode in ["train","train_detection_refine"] else False
    ...
    elif mode == "train_detection_refine":
        from train_epoch_detection_refine import train
        train(cfg=cfg, trainer=trainer, scheduler=scheduler, checkpoint=checkpoint) #, validate=False) #,warmup_scheduler=warmup_scheduler
    elif mode == "pretrain_rgb":
        from train_epoch_rgb_gt import train
        train(cfg=cfg, trainer=trainer, scheduler=scheduler,
              checkpoint=checkpoint)  # , validate=False) #,warmup_scheduler=warmup_scheduler
